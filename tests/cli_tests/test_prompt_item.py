...
class TestPromptItemCreationAndProcess:
    ...
    def test_process_item_ok(
        self, input_value, required_item, mocked_item_methods
    ):
    ...
    def test_process_item_failed_validation(
        self, required_item, mocked_item_methods
    ):
...
class TestPrompItemValidate:
    ...
    def test_validate_required_with_no_validators(
        self, input_value, required_item
    ):
    ...
    def test_validate_with_validators(self, required_item, item_parsers):
        validators = item_parsers(side_effect=VF_SIDE_EFFECT, params=PARAMS)
    ...
    def test_validate_with_errors(self, required_item, item_parsers):
        ...
        validators = item_parsers(side_effect=side_effect, params=PARAMS)
        validator_func = validators[0].apply
        required_item.validators = validators

        with pytest.raises(ValidationError):
            required_item._validate(INPUT1)
...
class TestPrompItemTransform:
    ...
    def test_transform_transformers(self, required_item, item_parsers):
        transformers = item_parsers(side_effect=TR_SIDE_EFFECT1, params=PARAMS)
    ...
    def test_transform_with_errors(self, required_item, item_parsers):
        ...
        transformers = item_parsers(side_effect=side_effect, params=PARAMS)
        transformer_func = transformers[0].apply
        required_item.transformers = transformers

        with pytest.raises(TransformationError):
            required_item._transform(INPUT1)
