...
class CommitHistoryMirror:
    ...
    async def reflect(
        ...
        if not self.dest_has_tree and dest_branch:
            ...
            raise ValueError(
                "Destination repo currently does not have a "
    ...
    async def _replicate(self, commits: Generator[Commit, None, None]) -> None:
        """Wraps replication as async tasks and runs them in parallel."""
        logger.debug("Preparing commit replication tasks")
        tasks = [self._commit(i, commit) for i, commit in enumerate(commits)]
    ...
    def _check_mirror_state(self):
        ...
        logger.debug("Checking mirror status of destination repo")
